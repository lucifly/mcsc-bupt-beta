...
$(function() {
    ...
    var IframeworkView = Backbone.View.extend({
        ...
        frameCount: 0,
        // HACK to not use same name in Firefox
        NativeNodes: {},
        plugins: {},
        events: {
            "click .close": "closePanels",
            "click .close2": "closePanels2",

            "click .show-load": "showLoad",
            "click .show-load2": "showLoad2",
        ...
        saveGist: function() {
            ...
            // Button
            this.$(".savegist").prop('disabled', true).text("saving...");
        ...
        updateCurrentInfo: function() {
            ...
            this.$(".currentapp .seturl").text(decodeURIComponent(graph["info"]["url"]));
            ...
            if (graph.info.hasOwnProperty("parents")) {
                ...
                if (parents.length > 0) {
                    ...
                    if (split.length > 0) {
                        ...
                        var gistLink = $('<a title="your saved gist" target="_blank" class="share icon-github"></a>').attr("href", last);
                        var fbLink = $('<a title="share on facebook" target="_blank" class="share icon-facebook-rect"></a>').attr("href", 'https://www.facebook.com/sharer.php?u=' + gisturlE + '&t=' + titleE);
                        ...
                        tweet = gisturl + tweet;
                        var twitterLink = $('<a title="post to twitter" target="_blank" class="share icon-twitter-bird"></a>').attr("href", 'https://twitter.com/intent/tweet?text=' + encodeURIComponent(tweet));

                        this.$(".currentapp .permalink").empty().append(gistUrlSelect).append(" ").append(gistLink).append(fbLink).append(twitterLink);
                    }
                }
            }
...
$(function() {
    ...
    Iframework.Graph = Backbone.Model.extend({
        ...
        removeNode: function(node) {
            ...
            this.eventsHistory.add(new Iframework.Event({
                action: "removeNode",
                args: {
                    "node": node,
                    "edges": connected
...
$(function() {
    ...
    Iframework.NodeBoxIframeView = Iframework.NodeBoxView.extend({
        ...
        initialize: function() {
            ...
            // Add refresh button
            this.$("button.remove").after($('<button title="reload iframe" type="button" class="refresh icon-cw"></button>'));
            // Add refresh event
...
$(function() {
    ...
    Iframework.PortView = Backbone.View.extend({
        ...
        clickhole: function(event) {
            ...
            // Close button
            popupEl.children("button.close").button({
                icons: {
                    primary: "icon-cancel"
                },
                text: false
            ...
            if (isIn) {
                ...
                if (showForm) {
                    inputForm.append($("<button></button>").html("send").attr({
                        "type": "submit",
                        "class": "send",
...
$(function() {
    ...
    Iframework.PortInView = Iframework.PortView.extend({
        ...
        clickhole: function(event) {
            ...
            if (showForm) {
                inputForm.append($("<button></button>").attr({
                    "type": "submit",
                    "class": "send icon-ok",
                    "title": "send value to module"
                }));
    ...
    Iframework.ModuleView = Backbone.View.extend({
        ...
        initialize: function() {
            ...
            if (this.model.isNative) {
                ...
                this.$(".addnode").addClass("isNative");
                //console.log(this.model.groupAndName[1]);
            }
            /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            else {
...
$(function() {
    ...
    var IframeworkRouter = Backbone.Router.extend({
        routes: {
            "example/:url": "loadExample",
            // #example/url
            "new": "newBlank",
            "local/:url": "loadLocal",
            "gist/https://gist.github.com/:user/:id": "loadGistUgly",
            // Redirects
            "gist/:id": "loadGist",
            "*path": "default"
        },
...
$(function() {
    var template = $(
    	'<div>' +
    		'<div class="sourceedit">' +
    			'<textarea id="noflotextarea"></textarea>' +
    		'</div>' +
    		'<div class="controls">' +
    			'<button class="button sourcerefresh icon-cw" title="refresh the source code">refresh</button>' +
    			'<button class="button sourcecompress icon-bag" title="refresh and compress the source code into one line">compress</button>' +
    ...
    // Add menu
    Iframework.addMenu("Noflo", template, "icon-cog");

    // On change update code view
    Iframework.on("change",
    function(graph) {
        if (Iframework.graph && Iframework.$(".menu-source").is(":visible")) { //class = menu-source
            // Bookmark to scroll back to
            var scrollBackTop = code.prop("scrollTop");
...
$(function() {
    ...
    Iframework.loadLibrary = function(library) {//载入module-library.js
        ...
        for (var category in library) {//每个category代表一个组件集合
            ...
            // section title
            section.append($('<h3><a href="#">' + category + "</a></h3>"));//section标签对应library里面的category

            // section items
            var sectionDiv = $("<div></div>");
            var modules = library[category];//modules为当前分类下的组件数组

            for (var i = 0; i < modules.length; i++) {//每一个module代表一个组件
        ...
        accordion2.children(".library-section").accordion({
            animate: true,
            header: "h3",
            heightStyle: "content",
            collapsible: true,
...
$(function() {
    ...
    var template = $(
    	'<div class="meemoo-plugin-images">' +
    		'<div class="listing">' +
    			...
    			'<span style="position:absolute;width:0px;overflow:hidden;">' +
    				'<input type="file" class="file-input-local" accept="image/*" multiple />' +
    			'</span>' +
    			...
    			'<div class="image-drop local-drop">' +
    				'<div class="drop-indicator">' +
    					'<p>drag image here to hold it</p>' +
    				'</div>' +
    			'</div>' +
    			'<div class="thumbnails local-listing"></div>' +
    				'<h2>Meemoo.me images (public)</h2>' +
    				'<span style="position:absolute;width:0px;overflow:hidden;">' +
    					'<input type="file" class="file-input-public" accept="image/*" />' +
    				'</span>' +
    				...
    				'<div class="info"></div>' +
    				'<div class="image-drop public-drop"><div class="drop-indicator">' +
    					'<p class="icon-globe-1">drag image here to save to meemoo.me</p>' +
    				'</div>' +
    			'</div>' +
    ...
    template.find(".local-drop").on("drop",
    function(event, ui) {
    ...
    var makeThumbnail = function(image) {
        ...
        var el = $('<div class="meemoo-plugin-images-thumbnail canvas">').append(image).draggable({
            cursor: "pointer",
            cursorAt: {
                top: -10,
                left: -10
    ...
    // Filepicker drop
    template.find(".public-drop").on("drop",
    function(event, ui) {
        if (!window.filepicker) {
    ...
    var imageTemplate = '<img crossorigin="anonymous" title="drag to graph or image node" />' + '<div class="controls">' + '<a class="link button icon-link" title="Open image in new window" target="_blank"></a>' + '<button class="export-public icon-export" title="Save straight to Flickr, Dropbox, Google, Facebook..."></button>' + '<button class="delete icon-trash" title="Delete image"></button>' + '</div>';
